...
class ActionPayCC(Action):
    ...
    def name(self) -> Text:
        ...
        return "action_pay_cc" #name of action
...
class ActionAddRecipient(Action):
     ...
     def name(self) -> Text: #name of action
...
class ActionShowTransferCharge(Action):
    ...
    async def run(
        ...
        if active_form_name:
            ...
            events.append(UserUtteranceReverted())  # keep the tracker empty for the predictions with form switch stories
           
            events.append(SlotSet("AA_CONTINUE_FORM", None))  #calls utter_ask_{form}_AA_CONTINUE_FORM, by making it the requested_slot
         
            events.append(FollowupAction(active_form_name))    # #bot get back to convo: next action
...
class ValidatePayCCForm(CustomFormValidationAction):
    ...
    async def validate_amount_of_money(
        ...
        tracker: Tracker,
        domain: Dict[Text, Any],
    ) -> Dict[Text, Any]:   #Validates value of 'amount-of-money' slot
      
        if not value:
            return {"amount-of-money": None}

        account_balance = profile_db.get_account_balance(tracker.sender_id)
        if type(value) is str:         # check if user asked to pay the full or the minimum balance
    ...
    async def validate_credit_card(
        ...
        tracker: Tracker,
        domain: Dict[Text, Any],
    ) -> Dict[Text, Any]:  #Validates value of 'credit_card' slot
...
class ValidateTransferMoneyForm(CustomFormValidationAction):
    ...
    async def validate_PERSON_transfer(
        ...
        if isinstance(value, list):        # picks first one if both Spacy & DIET extracted the PERSON
            value = value[0]
...
class ActionSessionStart(Action):
    ...
    def _slot_set_events_from_tracker(
        ...
        #Fetches SlotSet events from tracker and carries over keys and values # when restarting slots should be reset
