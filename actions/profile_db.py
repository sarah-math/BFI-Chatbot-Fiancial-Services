...
GENERAL_ACCOUNTS = {
    ...
    "vendor": ["carrfours", "geant", "cosmitto"],
    "depositor": ["work", "side-job"],
...
class ProfileDB:
    ...
    def get_account_from_session_id(self, session_id: Text):
        #Get an Account from the `Account.session_id , repopulate the db if action server is down in middle of convo
    ...
    def get_account_number(account: Union[CreditCard, Account]):
        #Get a bank or credit card account number by adding the number of leading zeros based on `Account.id`"""
    ...
    def add_credit_cards(self, session_id: Text):
        #opulate the creditcard table for a given session_id
    ...
    def populate_profile_db(self, session_id: Text):
        #Initialize the database for a conversation session with sample values.
